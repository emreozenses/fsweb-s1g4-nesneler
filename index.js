...
MenuElemaniOlustur("kofte", 5, "sicak");
MenuElemaniOlustur("sucuk", 2, "baharatli");
MenuElemaniOlustur("sis", 10, "kebap");
console.log(MenuElemaniOlustur("kofte", 5, "sicak"));
console.log(MenuElemaniOlustur("sucuk", 2, "baharatli"));
console.log(MenuElemaniOlustur("sis", 10, "kebap"));
...
const burger = {
  ...
  indirim: function (meslek) {
    if (meslek === "öğretmen") {
      const indirimliFiyat = this.fiyat * 0.75;
      return indirimliFiyat;
    } else if (meslek === "öğrenci") {
      const indirimliFiyat = this.fiyat * 0.75;
      return indirimliFiyat;
    } else if (meslek === "diğer") {
      const indirimliFiyat = this.fiyat * 0.9;
      return indirimliFiyat;
    }
  },
};
...
function AnahtardanDegerlendirmeAl(arr, indexNumber) {
  const mesaj =
    arr[indexNumber].isim +
    " isimli kişi " +
    arr[indexNumber].puan +
    " puan verdi ve şunları yazdı: " +
    arr[indexNumber].geribildirim;
  console.log(mesaj);
  return mesaj;
}
...
function SonDegerlendirmeyiAl(arr) {
  const mesaj =
    arr[arr.length - 1].isim +
    " isimli kişi " +
    arr[arr.length - 1].puan +
    " puan verdi ve şunları yazdı: " +
    arr[arr.length - 1].geribildirim;
  console.log(mesaj);
  return mesaj;
}
